...
The dataset used for this project is the "Telco Customer Churn" dataset, which contains information about customers of a telecom company, including their demographics, account information, and whether they have churned. The dataset includes features such as customer ID, gender, senior citizen status, partner status, dependents, tenure, phone service, multiple lines, internet service, online security, online backup, device protection, tech support, streaming TV, streaming movies, contract type, paperless billing, payment method, monthly charges, total charges, and churn status. The dataset is commonly used for churn prediction tasks and provides a rich set of features for building predictive models

## Evaluation Metrics

The model is evaluated using the following metrics:

- **AUC-ROC**: Measures the model's ability to distinguish between churners and non-churners.
- **Recall**: Measures the proportion of actual churners correctly identified by the model.
- **Precision**: Measures the proportion of predicted churners that are actually churners.
...
- Tested data cleaning functions.
- Tested my pipeline and model using pytest.
- Tests are located in the `tests/` directory.

## Model Packaging and Deployment Readiness

- The model and preprocessing pipeline are packaged using joblib for easy loading and inference.
- Configuration files are saved in JSON format for reproducibility.
- Deployment scripts and instructions are provided for integration into production systems using FastAPI
